...
class OcrHandle(object):
    ...
    def crnnRecWithBox(self, im, boxes_list, score_list, with_count):
        """
        crnn模型，ocr识别
        @@model,
        @@converter,
    ...
    def text_predict(self,img,short_size, with_count=True):
        boxes_list, score_list = self.text_handle.process(np.asarray(img).astype(np.uint8),short_size=short_size)
        result = self.crnnRecWithBox(np.array(img), boxes_list,score_list, with_count)
        return result
